...
- ~~`docs/09 §8` の GUI スモークテスト最終一巡~~ → **静的デバッグ完了**（`QUALITY_REPORT.md`
  「`docs/09 §8` 11-step smoke — static pass」表。11 ステップ全てをコードパス + 通過済み結合
  テストに対応づけ、v0.0.3 差分の相互作用を確認。§6/§7 チェックリストも静的スポット確認。
  Live Illustrator ドロワーに欠けていた `aria-live="polite"` を追加）。
