//! The thin IPC layer: `#[tauri::command]` functions only. Business logic lives in
//! `services/`. Every command returns `Result<T, AppError>` and takes domain-shaped
//! arguments (docs/02 §4).
...
/// Cheap startup probe the frontend can call to confirm the backend is alive and
/// `app.db` is readable.
#[tauri::command]
