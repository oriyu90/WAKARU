...
fn seed_project(env: &Env, name: &str, body: &str) -> String {
    ...
    pdb.execute(
        "UPDATE sources SET status='ready' WHERE id=?1",
        [&added.source.id],
    )
    .unwrap();
    pid
...
fn counts(root: &Path, pid: &str) -> (i64, i64, i64) {
    ...
    (
        db.query_row("SELECT count(*) FROM sources", [], |r| r.get(0))
            .unwrap(),
        db.query_row("SELECT count(*) FROM documents", [], |r| r.get(0))
            .unwrap(),
        db.query_row("SELECT count(*) FROM chunks", [], |r| r.get(0))
            .unwrap(),
    )
}
...
fn ac_10_1_export_import_restores_sources_documents_and_chunks() {
    ...
    let n: i64 = env
        .app_db
        .query_row(
            "SELECT count(DISTINCT project_id) FROM global_index",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
fn ac_10_3_a_future_major_schema_is_rejected() {
    ...
    let zip_path = dest.join("future.wakaru.zip");
    {
        ...
        z.start_file("manifest.json", SimpleFileOptions::default())
            .unwrap();
        z.write_all(br#"{"schemaVersion":"9.0.0","project":{"name":"X"}}"#)
            .unwrap();
        z.finish().unwrap();
    }
