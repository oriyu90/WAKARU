...
impl Env {
    ...
    fn project(&self) -> String {
        projects::create(
            &self.app_db,
            &self.projects_dir,
            CreateProjectInput { name: "p".into(), description: None, color: None },
        )
        .unwrap()
        .id
    }
    ...
    fn ingest_file(&self, pid: &str, path: &Path, kind: SourceKind) -> String {
        ...
        let out = ingest::run(&ctx, added.kind.unwrap_or(kind), &IngestInput::File(added.dest)).unwrap();
        pdb.execute(
...
fn ac_2_3_opening_a_source_twice_reuses_one_tab() {
    ...
    let pid = env.project();
    let tmp = tempfile::tempdir().unwrap();
    let sid = env.ingest_file(&pid, &write(tmp.path(), "a.md", "# h\n\nbody text here"), SourceKind::Markdown);
    let pdb = env.pdb(&pid);

    let t1 = viewer::open_tab(&pdb, &sid, None).unwrap();
    let t2 = viewer::open_tab(&pdb, &sid, Some(serde_json::json!({ "t": "page", "page": 2 }))).unwrap();
...
fn ac_2_6_tabs_persist_across_reopen() {
    ...
    let pid = env.project();
    let tmp = tempfile::tempdir().unwrap();
    let sid = env.ingest_file(&pid, &write(tmp.path(), "b.txt", "hello world"), SourceKind::Text);
...
fn source_detail_exposes_asset_urls_for_image_and_pdf_page_count() {
    ...
    image::RgbImage::from_pixel(64, 64, image::Rgb([10, 20, 30])).save(&p).unwrap();
    ...
    let url = detail.primary_asset_url.expect("image should have an asset url");
...
fn asset_resolver_only_serves_files_inside_the_project_sandbox() {
    ...
    let pid = env.project();
    let tmp = tempfile::tempdir().unwrap();
    let sid = env.ingest_file(&pid, &write(tmp.path(), "d.txt", "safe body"), SourceKind::Text);

    // The copied original is reachable.
    let ok = assets::resolve(&env.projects_dir, &format!("/{pid}/{sid}/sources/{sid}/d.txt"));
    assert!(ok.is_ok(), "{ok:?}");
    ...
    for bad in [
        ...
        assert!(assets::resolve(&env.projects_dir, &bad).is_err(), "should deny {bad}");
    }
}
